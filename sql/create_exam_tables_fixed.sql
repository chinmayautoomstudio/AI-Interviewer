-- Exam System Database Schema (Fixed for existing Supabase structure)
-- This file creates all necessary tables for the online examination system
-- Compatible with existing users, candidates, and job_descriptions tables
...
CREATE TABLE IF NOT EXISTS job_question_categories (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS exam_questions (
  ...
  created_by_user_id UUID REFERENCES users(id), -- Changed from admin_users to users
  status VARCHAR(20) DEFAULT 'approved' CHECK (status IN ('draft', 'approved', 'rejected')),
  hr_notes TEXT,
  last_modified_by UUID REFERENCES users(id), -- Changed from admin_users to users
...
CREATE INDEX IF NOT EXISTS idx_question_topics_parent ON question_topics(parent_topic_id);
CREATE INDEX IF NOT EXISTS idx_question_topics_category ON question_topics(category);
CREATE INDEX IF NOT EXISTS idx_question_topics_active ON question_topics(is_active);

CREATE INDEX IF NOT EXISTS idx_job_question_categories_job ON job_question_categories(job_description_id);
CREATE INDEX IF NOT EXISTS idx_job_question_categories_topic ON job_question_categories(topic_id);
CREATE INDEX IF NOT EXISTS idx_job_question_categories_priority ON job_question_categories(priority DESC);

CREATE INDEX IF NOT EXISTS idx_exam_questions_job ON exam_questions(job_description_id);
CREATE INDEX IF NOT EXISTS idx_exam_questions_category ON exam_questions(question_category);
CREATE INDEX IF NOT EXISTS idx_exam_questions_difficulty ON exam_questions(difficulty_level);
...
CREATE INDEX IF NOT EXISTS idx_exam_sessions_token ON exam_sessions(exam_token);
CREATE INDEX IF NOT EXISTS idx_exam_sessions_status ON exam_sessions(status);
CREATE INDEX IF NOT EXISTS idx_exam_sessions_expires ON exam_sessions(expires_at);

CREATE INDEX IF NOT EXISTS idx_exam_responses_session ON exam_responses(exam_session_id);
CREATE INDEX IF NOT EXISTS idx_exam_responses_question ON exam_responses(question_id);
CREATE INDEX IF NOT EXISTS idx_exam_responses_answered_at ON exam_responses(answered_at);
...
CREATE INDEX IF NOT EXISTS idx_exam_results_status ON exam_results(evaluation_status);
...
DROP TRIGGER IF EXISTS update_question_topics_updated_at ON question_topics;
CREATE TRIGGER update_question_topics_updated_at 
  BEFORE UPDATE ON question_topics 
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_job_question_categories_updated_at ON job_question_categories;
