...
-- Admin access policies (admin users can manage all exam data)
...
CREATE POLICY "Candidates can view their own exam sessions" ON exam_sessions FOR SELECT USING (candidate_id = auth.uid());
...
CREATE POLICY "Candidates can view their own exam results" ON exam_results FOR SELECT USING (candidate_id = auth.uid());

-- Candidates can update their own exam sessions (for status changes)
CREATE POLICY "Candidates can update their own exam sessions" ON exam_sessions FOR UPDATE USING (candidate_id = auth.uid());
...
CREATE TRIGGER update_job_question_categories_updated_at BEFORE UPDATE ON job_question_categories FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
